        ...
        if ($fsockopen!=false) {
            if (preg_match('/raw/i',$attr)) {
                ...
                foreach ($fileback as $key=>$file) {
                    ...
                    $package=json_decode(file_get_contents($repo.'.pkg'),true); $files=explode(';',$package['files']);
                ...
                recurseRmdir($repo); if (file_exists($repo.'.d')) {
                ...
                } $filerest=str_replace('./','',(glob('./*.txt.bak'))); foreach ($filerest as $key=>$file) {
                    if (file_exists($file)) {
                        chmod($file,0777);
                        rename($file,str_replace('.txt.bak','.txt',$file)); chmod(str_replace('.txt.bak','.txt',$file),0777);
                    }
                }
            }
        } chmod('get.lock',0777); unlink('get.lock');
    } elseif (($action=='r')||($action=='p')||($action=='m')) {
        if (preg_match('/raw/i',$attr)) { recurseRmdir($pkg);
        } else {
            if ((file_exists($pkg.'.pkg'))&&(@json_decode(file_get_contents($pkg.'.pkg'),true)!=null)) {
                $package=json_decode(file_get_contents($pkg.'.pkg'),true); $list=$package['files']; $files=explode(";",$list); foreach ($files as $key=>$file) {
                    if (file_exists($file)) {
                        chmod($file,0777); unlink($file);
                    }
