...
function snakeToCamel(str,delim='_') {
    ...
    var res='',itemArr=[],letters=[];
    for (idx in arr) {
        itemArr=arr[idx].split('');
        letters=[]; for (ltrIdx in itemArr) {
            if (ltrIdx<=0) {
                letters.push(itemArr[ltrIdx].toUpperCase());
            } else {
                letters.push(itemArr[ltrIdx].toLowerCase());
            }
        } res+=letters.join('');
    } return res;
}
...
class BaseNConverter {
    decbase(dec,alpha='0123456789abcdef') {
        let alps=alpha.split(''),base=alps.length;
        let div=dec,quot=0,rem=0,res='';
        while (div>0) {
    ...
    basedec(baseN,alpha='0123456789abcdef') {
        let alps=alpha.split(''),base=alps.length;
        let digits=baseN.split('').reverse(),res=0;
        for (let it in digits) {
            res+=alps.indexOf(digits[it])*base**it;
        } return (baseN=='')?0:res;
    }
}
const baseN=new BaseNConverter();
class CaesarCipher {
    encode(bin,offs=0,alpha='01',delim=' ') {
        let res=''; if ((bin!==undefined)&&(bin!='')) {
            let hex='',pos,off,pas; for (let i=0; i<bin.length; i++) {
                pos=Math.abs((bin.codePointAt(i)+Math.abs(offs))%1114112);
                pas=(i>0)?bin.codePointAt(i-1):'',off=baseN.decbase(pos,alpha);
                if (pas.toString(16).length<=4) { hex+=off+delim; }
            } res=hex.slice(0,-(delim.length));
        } else { res=''; } return res;
    }
    decode(hex,offs=0,alpha='01',delim=' ') {
        let res=''; if ((hex!==undefined)&&(hex.includes(delim))) {
            let bytes=[],pos,off,str=hex.split(delim);
            for (let i=0; i<str.length; i++) {
                pos=baseN.basedec(str[i],alpha);
