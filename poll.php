<?php
$name=$_REQUEST['name']; $select=$_REQUEST['select'];
$pollFile=json_decode(file_get_contents($name),true);
$pollRange=$pollFile['range']; $pollOptions=$pollFile['options'];
switch ($pollFile['type']) {
    case 'start_duel': case 'init_duel':
        ...
        $diff=($max-$min); if (isset($pollOptions[$select])) {
            if ($diff<$pollRange) { $pollOptions[$select]+=1; }
        } break;
    default:
...
file_put_contents($name,json_encode($pollFile));chmod($name,0777);
